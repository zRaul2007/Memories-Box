...
function trocarPainelMobile(painel) {
    ...
    if (painel === 'editor') {
        painelEditor?.classList.remove('painel-mobile-oculto');
        document.getElementById('painelLateral')?.classList.add('painel-mobile-oculto');
    } else {
        painelEditor?.classList.add('painel-mobile-oculto');
        document.getElementById('painelLateral')?.classList.remove('painel-mobile-oculto');

        // Scroll suave até a seção correta
        setTimeout(() => {
            if (painel === 'musica') containerMusica?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            if (painel === 'planos') containerPlanos?.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 50);
    }
}
...
async function abrirCaderno(id, titulo, permissao) {
    ...
    // Resetar nav mobile ao abrir caderno
...
document.getElementById('inputFoto')?.addEventListener('change', (e) => {
    ...
    leitor.onload = function (evt) {
        ...
        img.onload = function () {
            ...
            // Blindagem contra drag nativo do browser dentro do editor
            caixaDeTexto.addEventListener('dragstart', (e) => {
                if (e.target.tagName === 'IMG' || e.target.closest?.('.polaroid')) {
                    e.preventDefault();
                }
            });
...
// Redimensiona preservando o conteúdo (usado ao ATIVAR o modo desenho)
function redimensionarCanvas() {
    ...
    // Se as dimensões não mudaram, não faz nada — preserva o desenho
    ...
    // Restaura o conteúdo após redimensionar
    const imgRestore = new Image();
    imgRestore.onload = () => ctxDesenho?.drawImage(imgRestore, 0, 0);
    imgRestore.src = snapshot;
}

// Redimensiona SEM preservar — usado pelo listener do Firebase (que vai redesenhar do banco)
...
document.getElementById('btnModoDesenho')?.addEventListener('click', () => {
    ...
    if (modoDesenhoAtivo) {
        ...
        redimensionarCanvas(); // Preserva conteúdo existente
        modoBorracha = false;
...
function iniciarDesenho(e) {
    ...
    // Restaura o composite operation a cada traço (redimensionar reseta o contexto)
    ctxDesenho.globalCompositeOperation = modoBorracha ? 'destination-out' : 'source-over';
    ctxDesenho.strokeStyle = modoBorracha ? 'rgba(0,0,0,1)' : (document.body.classList.contains('dark-mode') ? '#fff' : '#000');
...
if (canvasDesenho) {
    ...
    canvasDesenho.addEventListener('touchstart', iniciarDesenho);
    canvasDesenho.addEventListener('touchmove', desenhar);
...
function iniciarRotinasDoCaderno() {
    ...
    onValue(ref(database, `cadernos/${cadernoAtualId}`), (snapshot) => {
        ...
        if (d) {
            ...
            // Clima persistente: restaura o botão ativo para todos
            if (d.clima) {
    ...
    document.querySelectorAll('.btn-humor').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Salva na presença (tempo real) E no caderno (persistente)
